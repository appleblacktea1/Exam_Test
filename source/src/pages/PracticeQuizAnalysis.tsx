...
const PracticeQuizAnalysis = () => {
  ...
  useEffect(() => {
    if (!state) {
      if (id) {
        ...
        const target = isPractice ? `/practice/${id}` : `/quiz/${id}`;
        navigate(target, { replace: true });
      } else {
        navigate('/', { replace: true });
      }
    }
  }, [state, navigate, id, location.pathname]);
  ...
  // --- [終極修正版] 返回路徑邏輯 ---
  const handleReturn = () => {
    // 1. 從 returnPath 取得當前的長 ID (例如 "110_110010_507_04")
    const pathParts = returnPath.split('/');
    const currentRecordId = pathParts[pathParts.length - 1];

    // 2. [核心策略]：去 mockCategories 找「正確的考卷 ID」
    let targetId = null;
      ...
      if (cat.groups) {
        for (const group of cat.groups) {
          for (const exam of group.exams) {
            // 邏輯：如果 歷史紀錄ID 包含了 資料庫中的 exam.id
            // 例如: "110_110010_507_04" includes "010_507_04" -> TRUE
            if (currentRecordId.includes(exam.id)) {
              targetId = exam.id;
              break outerLoop;
            }
          }
    ...
    // 3. 決定最終 ID (找不到則使用手動修復備案)
    const finalId = targetId || manualFixId(currentRecordId);

    // 4. 組合路徑並跳轉
    const basePath = pathParts.slice(0, pathParts.length - 1).join('/');
    navigate(`${basePath}/${finalId}`);
  };

  // 備用方案：手動字串切割
  const manualFixId = (fullId: string) => {
...
export default PracticeQuizAnalysis;
