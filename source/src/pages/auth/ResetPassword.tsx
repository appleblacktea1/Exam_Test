import React, { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { Mail, Lock, ArrowRight, Eye, EyeOff, AlertCircle, CheckCircle2 } from 'lucide-react';

const ResetPassword = () => {
  const navigate = useNavigate();
  const logoUrl = "data:image/png;base64,";

  const [formData, setFormData] = useState({ email: '', password: '', confirmPassword: '' });

  // --- 修改處 1: 拆分為兩個獨立的狀態 ---
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);

  const [error, setError] = useState(null);
  const [isSuccess, setIsSuccess] = useState(false);

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/;

  const handleSubmit = (e) => {
    e.preventDefault();
    setError(null);

    if (!emailRegex.test(formData.email)) return setError('請輸入有效的註冊電子郵件');
    if (!passwordRegex.test(formData.password)) return setError('新密碼需至少 8 位並包含英文字母與數字');
    if (formData.password !== formData.confirmPassword) return setError('兩次密碼輸入不一致');

    setTimeout(() => {
      setIsSuccess(true);
      setTimeout(() => navigate('/login'), 3000);
    }, 1500);
  };

  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center p-4 font-sans text-zinc-900">
      <div className="w-full max-w-md bg-white border border-zinc-200 rounded-[2.5rem] shadow-xl shadow-zinc-200/50 p-10 relative z-10">

        {!isSuccess ? (
          <>
            <div className="text-center mb-8">
              <img src={logoUrl} alt="Logo" className="w-20 h-20 mx-auto mb-4 rounded-2xl shadow-lg" />
              <h1 className="text-3xl font-black tracking-tight mb-2">重設您的密碼</h1>
              <p className="text-zinc-500 font-medium px-4">請驗證您的 Email 並設定新密碼</p>
            </div>

            {error && (
              <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600 text-sm font-bold">
                <AlertCircle size={18} /> {error}
              </div>
            )}

            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Email 欄位 */}
              <div className="relative">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={20} />
                <input
                  type="email" required placeholder="註冊時的電子郵件"
                  className="w-full pl-12 pr-4 py-4 bg-zinc-50 border border-zinc-200 rounded-2xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium"
                  onChange={e => setFormData({...formData, email: e.target.value})}
                />
              </div>

              {/* --- 修改處 2: 新密碼 (獨立開關) --- */}
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={20} />
                <input
                  type={showPassword ? "text" : "password"}
                  required
                  placeholder="輸入新密碼 (英數 8 位)"
                  className="w-full pl-12 pr-12 py-4 bg-zinc-50 border border-zinc-200 rounded-2xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium"
                  onChange={e => setFormData({...formData, password: e.target.value})}
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-indigo-600 transition-colors"
                >
                  {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
              </div>

              {/* --- 修改處 3: 確認新密碼 (獨立開關) --- */}
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={20} />
                <input
                  type={showConfirmPassword ? "text" : "password"}
                  required
                  placeholder="再次確認新密碼"
                  className="w-full pl-12 pr-12 py-4 bg-zinc-50 border border-zinc-200 rounded-2xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium"
                  onChange={e => setFormData({...formData, confirmPassword: e.target.value})}
                />
                <button
                  type="button"
                  onClick={() => setShowConfirmPassword(!showConfirmPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-indigo-600 transition-colors"
                >
                  {showConfirmPassword ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
              </div>

              <button type="submit" className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-2 transition-all active:scale-95 mt-4">
                重設密碼 <ArrowRight size={18} />
              </button>
            </form>
          </>
        ) : (
          <div className="text-center py-6">
            <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6">
              <CheckCircle2 size={40} />
            </div>
            <h2 className="text-2xl font-black text-zinc-900 mb-2">密碼重設成功！</h2>
            <p className="text-zinc-500 font-medium mb-10">
              帳號 <span className="text-zinc-900 font-bold">{formData.email}</span> 的密碼已更新。<br/>
              正在跳轉至登入頁面...
            </p>
            <Link to="/login" className="text-indigo-600 font-bold hover:underline underline-offset-4">
              直接前往登入
            </Link>
          </div>
        )}

        <div className="mt-8 text-center">
          {!isSuccess && (
            <Link to="/login" className="text-sm font-bold text-zinc-400 hover:text-indigo-600 transition-colors">
              返回登入
            </Link>
          )}
        </div>
      </div>
    </div>
  );
};

export default ResetPassword;
