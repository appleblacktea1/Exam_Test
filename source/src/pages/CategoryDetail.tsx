...
import {
  ...
  Heart, ChevronDown, LayoutGrid, Zap // 注入 Zap 圖標
} from 'lucide-react';
...
export default function CategoryDetail() {
  ...
  const [favoriteExamIds, setFavoriteExamIds] = useState<Set<string>>(new Set());
  const [openGroups, setOpenGroups] = useState<Set<number>>(new Set([0]));

  if (!category) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-4">
        <h1 className="text-xl font-bold mb-4">找不到此分類</h1>
  ...
  const toggleFavorite = (e: React.MouseEvent, examId: string) => {
    e.stopPropagation();
    setFavoriteExamIds(prev => {
            ...
            return (
              <div key={gIdx} className="space-y-2">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl shadow-sm hover:border-indigo-500/30 transition-all"
                >
                ...
                <div className={cn(
                  ...
                  <div className="min-h-0 space-y-3 px-1">
                    {group.exams.map((exam) => {
                      const isLiked = favoriteExamIds.has(exam.id);
                      return (
                        <div
                          key={exam.id}
                          className="bg-white dark:bg-zinc-900/50 rounded-2xl p-5 border border-zinc-100 dark:border-zinc-800 shadow-sm"
                        >
                          {/* 上半部：資訊與收藏 */}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <button
                              ...
                              className="flex items-center justify-center gap-2 py-3 px-4 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-xl font-bold text-sm transition-all active:scale-95"
                            >
                              <Coffee size={16} />
                              一般模式
                            </button>
                            <button
                              onClick={() => navigate(`/challenge/${exam.id}`)}
                              className="flex items-center justify-center gap-2 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-500/20 transition-all active:scale-95"
                            >
